...
class Create extends Component
{
    ...
    checkValidation(fields) {
		...
		if(fields.name.length === 0 || fields.capacity.length === 0 || this.state.teachers.length === 0 || Object.keys(this.state.courseType).length === 0) {
			return true;
		} else {
			return false;
		}
			  	...
			  	<div className="modal-dialog" role="document">
			    	<div className="modal-content">
			      		<div className="modal-header">
			        		<h5 className="modal-title">Dodaj kurs</h5>
			        ...
			        <form onSubmit={this.handleInsert}>
			      		<div className="modal-body">
			          		<div className="form-group">
			            		<label htmlFor="name" className="col-form-label">Naziv:</label>
			            		<input type="text" className={`form-control form-control-sm ${this.hasErrorFor('name') ? 'is-invalid' : ''}`}
			            		 id="name" name="name" placeholder="Naziv" onChange={this.handleInputFieldChange} value={this.state.name}/>
			            		{this.renderErrorFor('name')}
			         	 	</div>
                            <div className="form-group">
			            		<label htmlFor="capacity" className="col-form-label">Kapacitet grupe:</label>
			            		<input type="text" className={`form-control form-control-sm ${this.hasErrorFor('capacity') ? 'is-invalid' : ''}`}
			            		 id="capacity" name="capacity" placeholder="Kapacitet grupe" onChange={this.handleInputFieldChange} value={this.state.capacity}/>
			            		{this.renderErrorFor('capacity')}
			         	 	</div>
                            <div className="form-group" style={{marginBottom: '5px'}}>
                                <label htmlFor="courseType" className="col-form-label">Naziv kandidata:</label>
                                <Dropdown style={{marginLeft: "10px", marginTop: "3px"}} className={`${this.hasErrorFor('courseType') ? 'is-invalid' : ''}`}>
                                    <Dropdown.Toggle title={Object.keys(this.state.courseType).length === 0 ? 'Naziv kandidata' : this.state.courseType.name} btnStyle="link"/>
                                    <Dropdown.Menu>
                            ...
                            <div className="form-group">
                                <label htmlFor="dateOfStart" className="col-form-label" style={{display: 'block'}}>Datum startovanja kursa:</label>
			      		...
			      		<div className="modal-footer">
			        		...
			        		<button type="submit" className="btn btn-primary btn-sm">Sačuvaj kurs</button>
			      		</div>
			   		</form>
			    	</div>
			  	</div>
